...
export async function createRecipient(input: {
  ...
  const { data, error } = await supabase
    ...
    .single()
  if (error) throw error
  // Seed a blank recipient_brain row so future updates have something to upsert against
  await supabase
    .from('recipient_brain')
    .insert({ recipient_id: (data as CareRecipient).id })
    .select('recipient_id')
    .maybeSingle()
  // Add the creator to care_teams as the owner
  await supabase.from('care_teams').insert({
    care_recipient_id: (data as CareRecipient).id,
